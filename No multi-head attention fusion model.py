...
def train_student_model(student_model, audio_teacher, text_data, audio_data,
                        epochs=14, save_path="Z:\\2024program", batch_size=2, alpha=0.45, true_labels=[]):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(batch_text_data), batch_size):
            ...
            # 交叉熵损失
            print(student_outputs.squeeze(1).shape)
            print(real_labels_tensor.shape)
    ...
    # 保存模型
    torch.save(student_model.state_dict(), save_path+'\\Bert_model.pth')
